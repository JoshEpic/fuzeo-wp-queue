 ...
 */
final class MemoryDriver implements QueueDriver
{
    ...
    public function reserve(ReserveRequest $request): ?Reservation
    {
        ...
        $reserved = $selected->withState(JobState::Reserved);
    ...
    public function size(string $queue): int
    {
        ...
        $count = 0;
        foreach ($this->jobs as $envelope) {
            if ($envelope->queue === $queue && $envelope->state === JobState::Pending) {
                $count++;
            }
        }
    ...
    private function recoverExpired(\DateTimeImmutable $now): void
    {
        foreach ($this->reservations as $jobId => $reservation) {
            if ($reservation->isExpired($now)) {
                $this->jobs[$jobId] = $reservation->envelope->withState(JobState::Pending);
