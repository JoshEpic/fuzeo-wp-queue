...
- **WordPress compatibility mode** is bounded and degraded; it is not a worker fleet.
...
- **First-autoloader-wins** when multiple plugins bundle Queue. Constrain `^1.2` or `^1.0` and do not scope the namespace.
...
- **Shared hosting without long-running CLI** should use documented cron or compatibility modes. See [shared-hosting.md](shared-hosting.md).
